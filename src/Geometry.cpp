...
void Geometry::MakeTriangle(unsigned int vert0, unsigned int vert1, unsigned int vert2)
{
	...
	m_normals[vert0 * 3 + 0] = normal.x;
	m_normals[vert0 * 3 + 1] = normal.y;
	m_normals[vert0 * 3 + 2] = normal.z;
	m_normals[vert1 * 3 + 0] = normal.x;
	m_normals[vert1 * 3 + 1] = normal.y;
	m_normals[vert1 * 3 + 2] = normal.z;
	m_normals[vert2 * 3 + 0] = normal.x;
	m_normals[vert2 * 3 + 1] = normal.y;
	m_normals[vert2 * 3 + 2] = normal.z;

	// Compute a tangent
	m_tangents[vert0 * 3 + 0] = tangent.x;
	m_tangents[vert0 * 3 + 1] = tangent.y;
	m_tangents[vert0 * 3 + 2] = tangent.z;
	m_tangents[vert1 * 3 + 0] = tangent.x;
	m_tangents[vert1 * 3 + 1] = tangent.y;
	m_tangents[vert1 * 3 + 2] = tangent.z;
	m_tangents[vert2 * 3 + 0] = tangent.x;
	m_tangents[vert2 * 3 + 1] = tangent.y;
	m_tangents[vert2 * 3 + 2] = tangent.z;

	// Compute a bi-tangent
	m_biTangents[vert0 * 3 + 0] = bitangent.x;
	m_biTangents[vert0 * 3 + 1] = bitangent.y;
	m_biTangents[vert0 * 3 + 2] = bitangent.z;
	m_biTangents[vert1 * 3 + 0] = bitangent.x;
	m_biTangents[vert1 * 3 + 1] = bitangent.y;
	m_biTangents[vert1 * 3 + 2] = bitangent.z;
	m_biTangents[vert2 * 3 + 0] = bitangent.x;
	m_biTangents[vert2 * 3 + 1] = bitangent.y;
	m_biTangents[vert2 * 3 + 2] = bitangent.z;
}
